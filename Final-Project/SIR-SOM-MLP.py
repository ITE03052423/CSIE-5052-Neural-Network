...
#             self.wi.append(makeMatrix(self.nLayer[i]+1, self.nLayer[i+1]))
    ...
    def backPropagate2(self, diffC,sameC, N, M):
        ...
        change1 = np.dot(y1[self.Layers-2].reshape((len(y1[self.Layers-2]),1)),deltas1[self.Layers-2].reshape((1,len(deltas1[self.Layers-2]))))        
        change2 = np.dot(y2[self.Layers-2].reshape((len(y2[self.Layers-2]),1)),deltas2[self.Layers-2].reshape((1,len(deltas2[self.Layers-2]))))        
...
#         print('##G',G.shape)
    ...
    def get_max_and_min_distance_pair(self,x,target,m,f):
        ...
        if f==True:
            print('min distance: %.4f  min distance(highest record):%.4f'%(np.sqrt(min_dist),np.sqrt(self.min_dist_ever)))
...
#         self.Y_m=np.array(self.Y_m)
        ...
        for m in range(len(self.MLP)):
            ...
            for echo_count in range(0,echo):
                ...
                self.MLP[m].backPropagate2([self.Y_m[m][min_dist_pair[0]],self.Y_m[m][min_dist_pair[1]]],[self.Y_m[m][max_dist_pair[0]],self.Y_m[m][max_dist_pair[1]]],eta_att,eta_rep)
...
if __name__ == '__main__':
    ...
    trainning_data,target_t=np.split(trainning_data[:],[len(trainning_data[0])-10],axis=1)
    target=[]
    ...
    demo_som_perceptron.train(trainning_data,target,0.00001,0.0001,4000)
    with open('./best_W_MLP.dat', 'w+') as f:
        ...
        for nn in demo_som_perceptron.best_NNw:
            ...
            i+=1
    with open('./min_dist_MLP.dat', 'w+') as f:        
        for m in range(len(demo_som_perceptron.MLP)+1):
            min_dist=np.sqrt(demo_som_perceptron.min_dist_diff_class(target,10,m))
            for i in range(min_dist.shape[1]):
#                 print(i,min_dist[i].flatten().shape)
                f.write('{}\n'.format(','.join(list(map(lambda x: str(x), min_dist[i])))))
